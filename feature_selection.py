...
def train_feature_selection(dataframes, estimators, dataset_type, positive_sensibility=0.75, negative_sensibility=0.55, k=50):
    ...
    for carbohydrate, X in dataframes.items():
        ...
        # display(output.loc[output.groupby("Carbohydrates")["F1 score class 1"].idxmax()])
...
def find_best_parameters(datasets, param_grid, dataset_type):
    """
        Estimators=0 sta ad indicare l'uso dell'albero decisionale piuttosto della random forest. 
        Se specificato un numero diverso, esso corrisponde al numero di stimatori della rf.
    """
