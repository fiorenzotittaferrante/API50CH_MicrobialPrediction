...
def process_data(dataset_folder):
    """
    Loads and preprocesses feature and label datasets from the specified folder.

    Args:
        dataset_folder (str): Path to the folder containing 'X.xlsx' and 'Y.xlsx'.

    Returns:
        tuple: (X, y) DataFrames where X contains binarized features and y contains cleaned labels.
    """
...
def get_balancing(y, draw=False, threshold_rebalance=0.4, threshold_OD=0.2):
    """
    Computes label balance and optionally plots class distributions.
    """
...
def divide_dataset(y, balancing, threshold_rebalance=0.4, threshold_OD=0.2):
    """
    Divides labels into balanced, unbalanced (OD), and resampled sets based on prevalence.

    Args:
        y (pd.DataFrame): Label dataset with binary values (0 or 1).
        balancing (pd.DataFrame): DataFrame containing label prevalence and balance info.
        threshold_rebalance (float, optional): Min prevalence to trigger resampling. Default is 0.4.
        threshold_OD (float, optional): Min prevalence to classify as unbalanced (OD). Default is 0.2.

    Returns:
        tuple: (y_balanced, y_od, y_resampled) DataFrames corresponding to each category.
    """
...
def downsampling(X, y, threshold_rebalance, threshold_OD, draw=False):
    ...
    Args:
        ...
        y (pd.DataFrame): Label dataset with binary values (0 or 1).
        threshold_rebalance (float): Min prevalence required to attempt resampling.
        threshold_OD (float): Min prevalence required to attempt feature selection.
        draw (bool, optional): If True, plots class distributions. Default is False.
...
def generate_dataset(dataset_folder, draw=False, threshold_rebalance=0.4, threshold_OD=0.2):
    """
    Generates balanced, semi-balanced, and unbalanced datasets from raw data.

    Args:
        dataset_folder (str): Path to the folder containing input dataset files.
        draw (bool, optional): If True, plots class distributions. Default is False.
        threshold_rebalance (float, optional): Min prevalence to attempt resampling. Default is 0.4.
        threshold_OD (float, optional): Min prevalence to attempt feature selection. Default is 0.2.

    Returns:
        dict: Dictionary with keys 'balanced', 'semibalanced', 'unbalanced', each containing 
              DataFrames with features and corresponding labels.
    """
